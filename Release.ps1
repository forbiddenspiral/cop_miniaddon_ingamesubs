...
$workspace_folder = "$env:USERPROFILE\Documents\STALKER-PROJECTS\COP-PROJECT"
$release_folder = "$workspace_folder\COP_RELEASE"
$project_folder = "$workspace_folder\COP-MAIN-PROJECT"
...
# Define the destination folder for all the final files.
$final_folder = "Mini-addon_InGameSubs_$patch_version-for_PRP_v1.2"

# Create the folder for the files if it doesn't exist already.
...
If ($enable_release -eq "yes") {
	...
	# Clone the project folder to a temporary location.
