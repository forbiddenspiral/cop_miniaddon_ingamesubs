...
param (
  ...
  [string]$RepositoryPath = "$env:USERPROFILE\Documents\STALKER\INGAMESUBS-MODS\COP-MODDING-FOLDER\COP-MAIN-PROJECT",
  # Set the location where the mod is built and stored as a folder together with a compressed 7z file.
  [string]$BuildPath = "$env:USERPROFILE\Documents\STALKER\INGAMESUBS-MODS\COP-MODDING-FOLDER\COP_MAIN_RELEASE"
)
...
function Build-Mod {
  ...
  if (-not ($missing_folder)) {
    ...
    $build_folder = "InGameSubs_Mod_$build_version-for_PRP_v1.2"
...
while ($true) {
  Write-Host "InGameSubs mod." -ForegroundColor Blue
